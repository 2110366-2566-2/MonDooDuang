...
export default function AppointmentListDetail({
  ...
  return (
    <div className="w-full flex flex-row bg-white/[0.94] rounded-xl p-3">
      ...
      <div className="flex flex-col text-base">
        ...
        <div className="flex flex-row font-light">
          ...
          <p className={`${ (event_role === "CUSTOMER") ? "text-[#E79900]" : "text-[#417D9F]"} `}>{showedTime(appointment_start_time)} - {showedTime(appointment_end_time)}</p>
        </div>
      </div>
    </div>
  )
